...
def cv_links(ui, prefix, lang):
    url = prefix+PROFILE['cv'][lang]
...
def body(page,lang,prefix):
    ...
    if page=='index':
        h=u['home']
        actions='<div class="social-links">'+link('research.html',u['view_research'],'social-button primary')+link(prefix+PROFILE['cv'][lang],u['download'],'social-button','download="Mateus-Benvenuti-CV.pdf"')+'</div>'
    ...
    if page=='background':
        ...
        return f'<article class="text-page"><h1>{u["nav"][page]}</h1><h2>{b["title"]}</h2><ol class="timeline">{timeline}</ol><section><h2>{b["skills"]}</h2><h3>{b["programming"]}</h3>{tags(b["skill_tools"])}<h3>{b["statistics"]}</h3>{tags(b["skill_stats"])}</section><h2>{b["languages"]}</h2><p>{b["language_text"]}</p>{cv_links(u,prefix,lang)}</article>'
    if page=='contact':
        c=u['contact']
        return f'<article class="text-page"><h1>{u["nav"][page]}</h1><p>{c["intro"]}</p><p>{link("mailto:"+PROFILE["email"],PROFILE["email"],"email-link")}</p><p class="detail">{c["location"]}</p>{contacts(u,prefix,True)}<h2>{c["documents"]}</h2>{cv_links(u,prefix,lang)}</article>'
    info=u[page]
...
for lang in ['en','pt']:
    ...
    entries.extend([
        ...
        {'title':u['view_cv'],'url':('../' if lang=='pt' else '')+PROFILE['cv'][lang],'description':u['cv_note']},
    ])
